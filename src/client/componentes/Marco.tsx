...
export function Marco({ titulo, atras = false, accion, claseMain, children }: MarcoProps) {
  ...
  return (
    <div className="fixed inset-0 flex h-[100dvh] flex-col overflow-hidden bg-papel select-none">
      <header
        style={color === undefined ? undefined : { backgroundColor: color }}
        className={`area-segura-arriba z-40 shrink-0 select-none border-b backdrop-blur-md touch-none ${
          color === undefined ? 'border-borde bg-papel/90' : 'border-white/20 text-white'
        }`}
      >
        <div className="mx-auto flex w-full max-w-lg items-center gap-2 px-3 pb-2">
          {atras && (
            <button
              ...
              aria-label="Volver"
              onClick={() => navegar(-1)}
              className={`-ml-1 flex size-11 shrink-0 items-center justify-center rounded-xl transition ${
                color === undefined
                  ? 'text-tinta active:bg-papel-hundido'
